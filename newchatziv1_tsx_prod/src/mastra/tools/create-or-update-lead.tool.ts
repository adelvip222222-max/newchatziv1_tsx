...
export const createOrUpdateLeadTool = createTool({
  ...
  execute: async (input) => {
    ...
    const duplicateConditions = [
      ...
      input.ticketId && Types.ObjectId.isValid(input.ticketId) ? { ticketId: input.ticketId } : null,
      input.phone ? { phone: input.phone } : null,
      input.email ? { email: input.email } : null,
    ].filter(Boolean);

    const filter: Record<string, unknown> = duplicateConditions.length
    ...
    const lead = await Lead.findOneAndUpdate(
      filter,
      {
        $set: {
          ...
          ticketId: input.ticketId && Types.ObjectId.isValid(input.ticketId) ? input.ticketId : undefined,
          contactId,
          name: input.name || "",
          email: input.email || "",
          phone: input.phone || "",
          interest: input.interest || "",
          intent: input.intent || input.interest || "general",
    ...
    return {
      ...
      status: String(lead.status || input.status || "new"),
