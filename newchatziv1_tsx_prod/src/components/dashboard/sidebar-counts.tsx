...
function ProviderIcon({ provider, size = 13 }: { provider: string; size?: number }) {
  ...
  if (normalized === "telegram") return <FaTelegramPlane {...props} />;
  if (normalized === "whatsapp") return <FaWhatsapp {...props} />;
  if (normalized === "facebook") return <FaFacebookF {...props} />;
  if (normalized === "instagram") return <FaInstagram {...props} />;
...
export function SidebarCountsPanel({ collapsed }: { collapsed: boolean }) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(loadCounts, 12_000);
    return () => {
