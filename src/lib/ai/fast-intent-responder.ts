...
const HANDLED_INTENTS = new Set(["greeting", "thanks", "goodbye", "out_of_scope", "unclear"]);
...
export async function detectAndReplyFast(input: FastIntentResponderInput): Promise<FastIntentResult> {
  ...
  const systemPrompt = [
    ...
    "If the message is only a greeting, thanks, goodbye, clearly unrelated general-topic question, or too unclear to route, set handled to true and write one concise human reply.",
    "For out_of_scope, politely say you can help only with this business and invite the customer to ask about services, booking, pricing, policies, or support. Do not answer the general question itself.",
