...
export async function generateAiReplyLegacy(input: GenerateReplyInput, options: DirectReplyOptions = {}) {
  ...
  try {
    ...
    const fallback = setting?.fallbackMessage || "أواجه عطلًا مؤقتًا في توليد الرد، لكن وصلتني رسالتك. ممكن توضح لي طلبك بجملة قصيرة حتى أحاول أساعدك بأقرب إجابة؟";
  ...
  const reply = sanitizeCustomerReply(rawReply || setting?.fallbackMessage || "أحتاج إلى معلومة إضافية حتى أجيب بدقة. ما المنتج أو الخدمة التي تقصدها؟");
  ...
  if (replyFingerprint && replyFingerprint === lastAssistantFingerprint) {
    ...
    const finalRepairReply = "حتى لا أكرر نفس الكلام، خلّيني أساعدك بأقرب طريقة: اكتب لي النقطة التي تريدها تحديدًا، مثل السعر أو طريقة الطلب أو المشكلة التي تواجهها، وسأرد عليك مباشرة بالمعلومة المتاحة.";
