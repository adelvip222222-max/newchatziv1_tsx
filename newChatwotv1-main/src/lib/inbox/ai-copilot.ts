...
function shortReply(_text: string, intent: Intent) {
  if (intent === "sales") return "أكيد، أقدر أساعدك. ما الخطة أو الخدمة التي تريد معرفة تفاصيلها؟";
  if (intent === "billing") return "تم، سأراجع تفاصيل الدفع وأعود لك بالتحديث المناسب.";
  return "وصلت رسالتك، سأراجع التفاصيل وأرد عليك حالاً.";
}

function friendlyReply(_text: string, intent: Intent) {
  if (intent === "support") return "أكيد، خلّيني أساعدك خطوة بخطوة. أرسل لي آخر خطوة توقفت عندها أو صورة للخطأ إن وجدت.";
  return "أهلاً بك، وصلتني التفاصيل. سأراجعها الآن وأعطيك أوضح رد ممكن.";
}
